-- fix-222: task-template overhaul — new team taxonomy + dynamic-role co-assignees
-- + a project-level Schematic Designer.
--
-- Server side of the overhaul (the cat retirement + picker dedupe are pure
-- client TS). Three parts:
--   1. Migrate default_team 'Architecture' → 'Design Associate', except
--      schematic-prep templates (text mentions "schematic") → 'Schematic Team'.
--      Mirrors src/lib/taskTeam.ts migrateArchitectureTeam. (task_templates.cat
--      is deliberately LEFT ALONE — the column stays, just unused by the UI.)
--   2. Add projects.schematic_designer text[] (single now, multiple allowed).
--   3. Rewrite bp_create_project_with_permits to (a) persist schematic_designer,
--      (b) route default_team → assigned_to for the new taxonomy, and (c) resolve
--      dynamic co-assignee tokens (role:design_associate → the permit's DA,
--      role:design_manager → the DM paired with that DA in dm_da_groups,
--      role:schematic_designer → the project's schematic designer(s)) at task
--      instantiation. (a)/(b)/(c) mirror src/lib/taskTeam.ts — keep in lockstep.
--
-- The RPC body is the LIVE pg_get_functiondef with only those additions
-- (migrations/ is a partial record; prod eibnmwthkcuumyclyxoe is source of truth).

-- ---------------------------------------------------------------------------
-- 1. Migrate default_team off the retired 'Architecture' key.
-- ---------------------------------------------------------------------------
UPDATE public.task_templates
SET default_team = CASE
  WHEN text ILIKE '%schematic%' THEN 'Schematic Team'
  ELSE 'Design Associate'
END
WHERE default_team = 'Architecture';

-- ---------------------------------------------------------------------------
-- 2. Project-level Schematic Designer(s).
-- ---------------------------------------------------------------------------
ALTER TABLE public.projects
  ADD COLUMN IF NOT EXISTS schematic_designer text[] DEFAULT ARRAY[]::text[];

-- ---------------------------------------------------------------------------
-- 3. Create RPC — persist schematic designer, new team routing, token resolve.
-- ---------------------------------------------------------------------------
