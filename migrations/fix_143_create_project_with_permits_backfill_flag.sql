-- fix-143 (2026-06-09): backfill historical projects through the wizard.
--
-- Adds p_manually_placed + a MANUAL placement branch to
-- bp_create_project_with_permits. Background: the BP's auto-placement only
-- fires when its dd_start/dd_end are NULL. In backfill mode the wizard sends
-- explicit (Monday/Friday-snapped) DD dates for the BP, so auto-placement is
-- skipped — and before this change NO draw_schedule row was created at all.
-- Now, when the BP carries manual DD dates and a lead DA is set, the RPC builds
-- the lane from those dates and flags it manually_placed = p_manually_placed so
-- future auto-rebalancing leaves the deliberately-historical lane alone.
--
-- The signature gains a 7th parameter, so the old 6-arg function is dropped
-- first (a DEFAULT-valued overload would otherwise make 6-arg calls ambiguous).
-- p_manually_placed defaults false, so any caller that omits it still works.
--
-- Everything else is unchanged from fix-141
-- (fix_141_snap_to_monday_forward_rpc_guards.sql): the auto-place path still
-- Monday-aligns slot_start and writes a Monday end_week.
--
-- Applied to PROD via Supabase MCP apply_migration as
-- "fix_143_create_project_with_permits_backfill_flag". This file is the repo
-- record.

DROP FUNCTION IF EXISTS public.bp_create_project_with_permits(uuid, text, text, text, jsonb, jsonb);

CREATE OR REPLACE FUNCTION public.bp_create_project_with_permits(p_tenant_id uuid, p_address text, p_juris text, p_notes text, p_project_data jsonb, p_permits jsonb, p_manually_placed boolean DEFAULT false)
...
DECLARE
  ...
  -- fix-143: BP's manual (backfill) DD dates, captured for manual placement.
...
BEGIN
  ...
  FOR v_permit IN SELECT * FROM jsonb_array_elements(p_permits) LOOP
    ...
    IF v_permit_type = 'Building Permit' AND v_bp_permit_id IS NULL THEN
      ...
      ELSIF v_bp_dd_start IS NOT NULL AND v_bp_dd_end IS NOT NULL THEN
        -- fix-143: backfill — manual DD dates supplied for the BP. Capture for
        -- the manual-placement branch after the loop.
  ...
  ELSIF v_manual_dd_start IS NOT NULL AND v_manual_dd_end IS NOT NULL AND v_lead_da IS NOT NULL THEN
    -- fix-143: backfill manual placement. Build the lane from the BP's manual
    -- DD dates (already Monday/Friday-snapped client-side; re-snap defensively).
    -- manually_placed = p_manually_placed so auto-rebalance leaves it alone.
    INSERT INTO public.draw_schedule (tenant_id, project_id, da_assigned, start_week, end_week, dd_start, dd_end, status, manual_status, manually_placed)
