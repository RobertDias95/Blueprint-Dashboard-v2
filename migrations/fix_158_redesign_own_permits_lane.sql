-- fix-158 (2026-06-12): redesign own-permits branch — create the draw_schedule
-- lane from the Redesign DD Phase section, like the reuses branch already does.
--
-- Bug (Bobby, prod): 7603 8th Ave NW [Redesign 1] (20497c78…, reuses=false →
-- own permits) never got a Draw Schedule lane. Its only permit is a PPR
-- (da='Marc', dd 2026-03-09→13), and draw_schedule has 0 rows for the project.
--
-- Why the own-permits lane logic missed it: it is gated on
--   (a) v_permit_type = 'Building Permit'  (this project's permit is a PPR), and
--   (b) v_lead_da IS NOT NULL (the project-level lead_da, which fix-152 stopped
--       populating on redesigns — the Step-1 Lead DA picker is hidden).
-- Both gates fail, so no lane is inserted. The reuses=true branch, by contrast,
-- places its lane deterministically from p_redesign_dd_phase (fix-144).
--
-- Fix: in the own-permits branch, when p_redesign_dd_phase is supplied, place
-- the lane from it — the SAME insert the reuses branch uses — independent of
-- permit type and project lead_da. The wizard now renders the Redesign DD Phase
-- section (DA + dates) for BOTH branches and sends p_redesign_dd_phase for both.
--
-- Body is the LIVE prod definition (migrations/ is a partial record) with only
-- the own-permits lane-insert block extended. Applied to prod via MCP; repo
-- backstop.

CREATE OR REPLACE FUNCTION public.bp_create_project_with_permits(
  p_tenant_id uuid, p_address text, p_juris text, p_notes text,
  p_project_data jsonb, p_permits jsonb,
  p_manually_placed boolean DEFAULT false, p_redesign_dd_phase jsonb DEFAULT NULL::jsonb)
...
BEGIN
  ...
  -- fix-158: an own-permits REDESIGN places its lane from the Redesign DD Phase
  -- section (DA + dates), independent of permit type / project lead_da — the
  -- same insert the reuses branch uses above. Takes priority over the BP-based
  -- auto/manual placement (which only fires for non-redesign new projects).
  IF p_redesign_dd_phase IS NOT NULL THEN
