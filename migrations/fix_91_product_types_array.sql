-- fix-91: projects.product_type (text) → projects.product_types (text[]).
--
-- A site can legitimately carry multiple product types (SFR + Attached
-- Units + Cottages on one parcel). The single text column blocked that;
-- Bobby's wizard redesign also moves the field to a settings-managed
-- multi-select. This migration:
--   1. Adds projects.product_types text[] (DEFAULT '{}' NOT NULL).
--   2. Backfills from product_type — every non-null/non-empty value
--      becomes a single-element array.
--   3. Drops projects.product_type.
--   4. Updates the two RPCs that read/write the column
--      (bp_create_project_with_permits + bp_update_project_with_permits)
--      to read product_types as a jsonb array from their JSONB payload.
--   5. Seeds app_config.productTypeOptions with the v1 vocabulary +
--      the additions Bobby called out (Duplex/Townhouse/Condo). Mirrors
--      fix-75's projectTagOptions seed; AdminProjectsTab edits this key
--      via the existing tag/options UI.
--
-- Spot-check post-apply: every existing project should have either an
-- empty array (the originally-null rows) or a single-element array
-- containing its previous product_type value.
...
ALTER TABLE public.projects DROP COLUMN IF EXISTS product_type;

-- Settings-managed option list. AdminProjectsTab's tag editor reads
-- this same shape (app_config.value: jsonb array of strings).
INSERT INTO public.app_config (key, value)
VALUES (
  'productTypeOptions',
  '["SFR","SFR w/ Accessory Units","Attached Units","Cottages","Duplex","Townhouse","Condo"]'::jsonb
)
ON CONFLICT (key) DO NOTHING;

-- ============================================================
-- bp_create_project_with_permits — accept product_types jsonb array.
-- Body pulled from prod 2026-06-02 via MCP; only the product_type read
-- + INSERT clause change.
-- ============================================================
...
BEGIN
  ...
  -- fix-91: product_types arrives as a jsonb array. NULL / non-array
  -- → empty text[] (the column NOT NULL DEFAULT '{}' would also catch
  -- this, but explicit is clearer).
    ...
    VALUES (
      ...
      COALESCE(
        ...
        CASE
          ...
          THEN (NULLIF(v_permit->>'dd_end', '')::date + INTERVAL '14 days')::date
          ELSE NULL
        END
      ),
...
-- ============================================================
-- bp_update_project_with_permits — accept product_types jsonb array.
-- Body pulled from prod 2026-06-02 via MCP; only the product_type
-- patch clause changes (CASE-when-key-present pattern, parsing the
-- array out of the JSONB patch).
-- ============================================================
...
BEGIN
  ...
  -- fix-91: pre-parse product_types from the patch (only used when the
  -- patch carries the key). Same array-from-jsonb pattern as the
  -- create RPC; non-array / missing → ARRAY[]::text[].
