-- fix-210: redesign create must write the DD dates to the PERMIT, not just the
-- draw-schedule block.
--
-- Bug: a redesign sends its DD window in p_redesign_dd_phase, NOT in the permit
-- jsonb. The non-reuse path created the new Building Permit with the permit
-- jsonb's dd_start/dd_end (NULL for a redesign), and the fix-208 holistic unify
-- then snapped every BP to that NULL primary — so the permit's dd_start/dd_end/
-- target_submit stayed NULL even though the block was placed correctly. The
-- Project Overview DD strip reads the PERMIT, so it showed blank.
-- Repro: 123 N 48th St [Redesign 1] (data already repaired separately).
--
-- Fix (server-canonical, in bp_create_project_with_permits): in the non-reuse
-- path, after the create loop, when p_redesign_dd_phase IS NOT NULL, apply the
-- SAME window that drives the block to the project's non-sub Building Permits,
-- and set target_submit = dd_end + 21 explicitly (the permits AFTER trigger does
-- not reliably recompute it on a create-time dd_end UPDATE). The REUSE path
-- returns earlier and never reaches this code, so it is unaffected. The block
-- INSERT is unchanged. This is the fix-208 def + the marked fix-210 block.
