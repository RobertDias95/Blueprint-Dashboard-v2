-- fix-208: enforce ONE holistic project DD window on the draw schedule, and add
-- a detector RPC for divergence.
--
-- Bobby's standard: a project's draw-schedule block represents a single holistic
-- DD window for the WHOLE project, regardless of how many Building Permits it
-- has. Two projects diverged (10431 SE 19th St: 4 BPs / 2 distinct dd_starts;
-- 621 Daley St: 2 BPs / 2 distinct) because the create path set per-BP dd_start
-- WITHOUT unifying — only the FIRST Building Permit received the auto/manual DD
-- window; later BPs kept their own jsonb dd_start (or none).
--
-- Step-0 audit of every permits.dd_start/dd_end writer (live defs):
--   * bp_set_bp_dd_dates ............. project-wide UPDATE permits ... WHERE
--     project_id — holistic already.
--   * bp_update_draw_schedule_with_dd_sync ... project-wide UPDATE — holistic.
--   * bp_move_draw_schedule_da ....... project-wide UPDATE — holistic.
--   * bp_update_project_with_permits . does NOT touch dd_start at all.
--   * bp_update_redesign_dd_phase .... only moves the draw_schedule block.
--   * bp_create_project_with_permits . PER-BP dd_start from the jsonb — the BUG.
-- This migration fixes the source (create) and adds the detector. Data repair of
-- the two known offenders is handled separately (not here).

-- ── A. ENFORCE: unify all non-sub Building Permits at creation ──────────────
-- Live def of bp_create_project_with_permits with two surgical additions
-- (marked "fix-208"): v_primary_dd_start/v_primary_dd_end locals + a post-loop
-- UPDATE that snaps every non-sub Building Permit to the primary (first non-sub)
-- BP's window. Sub-permits (parent_permit_id NOT NULL) are excluded from both
-- the anchor pick and the unify so a child never drives or diverges the DD.
...
DECLARE
  ...
  -- fix-208: holistic DD unify locals.
...
BEGIN
  ...
  -- fix-208: ONE holistic project DD window. The loop above only gives the FIRST
  -- Building Permit the auto/manual window; any later BP kept its own jsonb
  -- dd_start (or none), which would diverge the project's DD. Snap every non-sub
  -- Building Permit to the primary (first non-sub) BP's window. Sub-permits
  -- (children) are excluded — they don't anchor or diverge project DD.
  IF v_bp_permit_id IS NOT NULL THEN
    ...
    -- Alias the table: an unqualified `project_id` here collides with the
    -- RETURNS TABLE(project_id ...) OUT variable (#variable_conflict error).
...
-- ── B. DETECTOR: flag any project whose draw-schedule block is off its primary
-- DD window, or whose non-sub Building Permits carry divergent dd_start. ──────
-- SECURITY INVOKER (relies on the tenant RLS on draw_schedule/projects/permits)
-- with an explicit auth_tenant_ids() guard for defence-in-depth. The anchor /
-- "primary" BP is type='Building Permit' AND parent_permit_id IS NULL, lowest id.
CREATE OR REPLACE FUNCTION public.bp_draw_schedule_dd_mismatches()
 RETURNS TABLE(
   project_id uuid,
   address text,
   da text,
   start_week text,
   end_week text,
   primary_dd_start date,
   distinct_dd_starts bigint,
   bp_count bigint,
...
AS $function$
  WITH bp AS (
    SELECT p.project_id,
           count(*) AS bp_count,
           count(DISTINCT p.dd_start) FILTER (WHERE p.dd_start IS NOT NULL) AS distinct_dd_starts
    FROM public.permits p
    WHERE p.type = 'Building Permit' AND p.parent_permit_id IS NULL
    GROUP BY p.project_id
  ),
  prim AS (
    SELECT DISTINCT ON (p.project_id) p.project_id, p.dd_start AS primary_dd_start
    FROM public.permits p
    WHERE p.type = 'Building Permit' AND p.parent_permit_id IS NULL
    ORDER BY p.project_id, p.id ASC
  )
  SELECT ds.project_id,
         pr.address,
         ds.da_assigned AS da,
         ds.start_week,
         ds.end_week,
         prim.primary_dd_start,
         bp.distinct_dd_starts,
         bp.bp_count,
         to_char(date_trunc('week', prim.primary_dd_start)::date, 'YYYY-MM-DD') AS expected_start_week
  FROM public.draw_schedule ds
  JOIN public.projects pr ON pr.id = ds.project_id
  LEFT JOIN bp   ON bp.project_id   = ds.project_id
  LEFT JOIN prim ON prim.project_id = ds.project_id
  WHERE pr.tenant_id = ANY (public.auth_tenant_ids())
    AND prim.primary_dd_start IS NOT NULL
    AND (
      ds.start_week IS DISTINCT FROM to_char(date_trunc('week', prim.primary_dd_start)::date, 'YYYY-MM-DD')
      OR bp.distinct_dd_starts > 1
    );
$function$;

REVOKE ALL ON FUNCTION public.bp_draw_schedule_dd_mismatches() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.bp_draw_schedule_dd_mismatches() TO authenticated, service_role;
